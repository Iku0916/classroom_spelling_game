...
Rails.application.routes.draw do
  ...
  resources :word_kits, only: %i[new create show index destroy edit update] do
    ...
    resources :word_cards, only: %i[new create index destroy edit update]
    resource :self_study, only: %i[show update] do
  ...
  resources :game_rooms, only: %i[create show update] do
    member do
      get :waiting
      patch :start
      get :start
    ...
    resource :game_play, only: %i[show update] do
      post :answer
      patch :update_score
      post :finish
      get :overall_result
